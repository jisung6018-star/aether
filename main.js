document.addEventListener('DOMContentLoaded', () => {
    ...
    if (qnaForm) {
        qnaForm.onsubmit = async (e) => {
            e.preventDefault();
            const input = qnaForm.querySelector('input');
            const message = input.value.trim();
            
            if (!message) return;

            try {
                if (!window._fb) throw new Error('Firebase not initialized');
                ...
                alert(currentLang === 'en' ? 'MESSAGE TRANSMITTED TO AETHER HUB.' : '에테르 허브로 문의가 전송되었습니다.');
                qnaForm.reset();
            } catch (error) {
                console.error('Failed to send inquiry:', error);
                alert(currentLang === 'en' ? 'TRANSMISSION FAILED. PLEASE TRY AGAIN.' : '전송에 실패했습니다. 다시 시도해주세요.');
            }
        };
    }
    ...
    document.querySelectorAll('[data-i18n="btn_access_repo"]').forEach(btn => {
        btn.onclick = () => {
            const fileName = 'Aether_Technical_Whitepaper.txt';
            const content = `# Aether Dynamics Technical Whitepaper v1.0\n\n` +
                `## 1. Introduction\n` +
                `This document provides a technical overview of the Aether Dynamics platforms, focusing on the FORGE and NEXUS series.\n\n` +
                `## 2. FORGE Platform (Industrial Autonomy)\n` +
                `- Payload: 250kg\n` +
                `- Degrees of Freedom: 48 Joints\n` +
                `- Battery Life: 24h+ (Modular swap)\n` +
                `- Material: High-strength titanium-steel alloy\n\n` +
                `## 3. NEXUS Platform (Cognitive Research)\n` +
                `- Degrees of Freedom: 64 Joints\n` +
                `- Sensors: 8K Lidar, Biometric feedback, Neural-link interface\n` +
                `- AI Core: Aether Intelligence 2.0\n\n` +
                `## 4. Integration Guide\n` +
                `Detailed SDK documentation can be found in the developer repository.\n\n` +
                `---\n` +
                `© 2024 Aether Dynamics. Mission-Critical Robotics.`;
            ...
            try {
                ...
                // Allow a small delay before revoking the URL to ensure download starts
                setTimeout(() => window.URL.revokeObjectURL(url), 100);
                
                alert(currentLang === 'en' ? 'DOCUMENTATION DOWNLOAD STARTED.' : '기술 문서 다운로드가 시작되었습니다.');
            } catch (error) {
                console.error('Download failed:', error);
                alert(currentLang === 'en' ? 'DOWNLOAD FAILED.' : '다운로드에 실패했습니다.');
            }
        };
    });
